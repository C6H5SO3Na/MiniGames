...
namespace  SaboriJoushi
{
	...
	bool  Object::Initialize()
	{
		...
		this->changeStateTime = GetRandom(3, 12); //仕事中状態からサボり確認の予備動作状態に変更するまでの時間の設定
	...
	void Object::Think()
	{
		...
		case State::JWork:						//仕事中状態
			if (this->moveCnt >= this->gameFps * this->changeStateTime) { nowState = State::JBeforeCheckingForSabori; } //サボり確認の予備動作状態へ モニターFPSにゲームが依存しないようにするために条件式に * GetFps() / (float)gameFps を追加する
			break;

		case State::JBeforeCheckingForSabori:	//サボり確認の予備動作状態
			if (this->moveCnt >= this->gameFps * this->changeStateTime) //モニターFPSにゲームが依存しないようにするために条件式に * GetFps() / (float)gameFps を追加する
			{
				//☆ランダムで遷移する状態を変更する
				//遷移する状態を決定
		...
		case State::JCheckForSabori:			//サボり確認状態
			if (this->moveCnt >= this->gameFps * this->changeStateTime) { nowState = State::JWork; } //仕事中状態へ モニターFPSにゲームが依存しないようにするために条件式に * GetFps() / (float)gameFps を追加する
			if (this->hasFoundPlayerSabori) { nowState = State::JFoundSabori; } //サボり発見状態へ
			break;

		case State::JFoundSabori:				//サボり発見状態
			if (this->moveCnt >= this->gameFps * 3) { nowState = State::JWork; } //仕事中状態へ モニターFPSにゲームが依存しないようにするために条件式に * GetFps() / (float)gameFps を追加する
			break;
		}
	...
	void Object::Move()
	{
		...
		case State::JWork:						//仕事中状態
			...
			{
				//時間設定
				this->changeStateTime = GetRandom(3, 12); //3～12秒
			}
			break;
		...
		case State::JBeforeCheckingForSabori:	//サボり確認の予備動作状態
			...
			{
				//時間設定
				this->changeStateTime = GetRandom(2, 4); //2～4秒
			}
			break;
		...
		case State::JCheckForSabori:			//サボり確認状態
			...
			{
				//時間設定
				this->changeStateTime = GetRandom(3, 5); //3～5秒
			}
